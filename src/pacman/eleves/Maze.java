...
public class Maze {
	...
	public Maze(String filename) throws MazeException {
		try {
			...
			while ((ligne = br.readLine()) != null) {
				...
				if (nbX == 0) {
					...
					throw new MazeException(
							"Wrong Input Format: all lines must have the same size");
				nbY++;
			}
			br.close();
			...
			while ((ligne = br.readLine()) != null) {
				...
				for (int x = 0; x < ligne.length(); x++) {
					...
					if (ligne.charAt(x) == 'o')
						...
						capsules[x][y] = false;
					if (ligne.charAt(x) == 'P') {
						pacman_start_x.add(x);
						pacman_start_y.add(y);
					}
					if (ligne.charAt(x) == 'G') {
						ghosts_start_x.add(x);
						ghosts_start_y.add(y);
					}
				}
				y++;
			}
			...
			if (pacman_start_x.size() == 0)
				throw new MazeException(
						"Wrong input format: must specify a Pacman start");

			// On verifie que le labyrinthe est clos
			for (int x = 0; x < size_x; x++)
				if (!walls[x][0])
					throw new MazeException(
							"Wrong input format: the maze must be closed");
			for (int x = 0; x < size_x; x++)
				if (!walls[x][size_y - 1])
					throw new MazeException(
							"Wrong input format: the maze must be closed");
			...
			e.printStackTrace();
			throw new MazeException("Probleme a la lecture du fichier: "
					+ e.getMessage());
		}
	}
