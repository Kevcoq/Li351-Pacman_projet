...
public class Test_Collaboratif_NoCommonCase 
{
		protected MazeWritable maze;
		protected int timestep;
		protected ArrayList<ArrayList<Path> > all_paths_pacmans;
		
		public Test_Collaboratif_NoCommonCase(MazeWritable maze,int timestep)
		{
			this.maze=maze;
		...
		public ArrayList<Path> choosePaths()
		{
			...
			while(flag)
			{
				...
				for(int i=0;i<pacmans_paths.size();i++)
					...
					{
						if (!flag)
							if (pacmans_paths.get(i).nbCommonCasesButNotLast(pacmans_paths.get(j))>0)
							{
								flag=true;
							}
					}
				if (flag)
				{
					pacmans_paths=new ArrayList<Path>();
				}
		...
		public static void main(String args[]) throws ParseException, ClassNotFoundException, InstantiationException, IllegalAccessException, MazeException
		{
			...
			Test_Collaboratif_NoCommonCase t=new Test_Collaboratif_NoCommonCase(new MazeWritable(layout_file), timestep);
			t.computeAllPaths();
